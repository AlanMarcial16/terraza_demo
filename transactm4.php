...
$conn4 = mysqli_connect($servername, $username, $password, "pruebar");
...
// Obtener la habitación de Mesa 4 de la base de datos "pruebar"
...
if (mysqli_num_rows($result) > 0) {
    ...
    if (mysqli_num_rows($resultReservaciones) > 0) {
        ...
        // Consulta SQL para obtener la suma de 'preciou' de la tabla 'ordenmesa4' en la base de datos "pruebar"
        ...
        if (mysqli_num_rows($resultTotal) > 0) {
            ...
            if (mysqli_query($conn2, $updateSql)) {
                ...
                header("Location: guardaticket4.php");
                exit; // Asegurarse de que el script se detenga aquí
            } else {
                echo "Error al actualizar 'cextras': " . mysqli_error($conn2);
            }
        } else {
            echo "No se pudo obtener el valor de 'total' desde la tabla 'ordenmesa4' en la base de datos 'pruebar'.";
        }
    ...
    echo "No se encontró la habitación 'Mesa 4' en la tabla 'mesas' de la base de datos 'pruebar'.";
}
?>
