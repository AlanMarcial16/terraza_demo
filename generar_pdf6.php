...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Terraza Alimentos saludables y'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Eco-amigables'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode(''), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto

$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Pedido: MESA 6'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
...
// Obtener información de la "Mesa 6" de la tabla "mesas"
$sql_mesa6 = "SELECT tipo, habitacion, comensal FROM mesas WHERE id = 6";
$result_mesa6 = $conn->query($sql_mesa6);

// Verificar si se obtuvieron resultados
if ($result_mesa6->num_rows > 0) {
    $row_mesa6 = $result_mesa6->fetch_assoc();
    $tipo_mesa6 = $row_mesa6['tipo'];
    $habitacion_mesa6 = $row_mesa6['habitacion'];
    $comensal_mesa6 = $row_mesa6['comensal'];

    // Mostrar la información en el ticket
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("$tipo_mesa6: $habitacion_mesa6"), 0, 1, 'C');
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Cliente: $comensal_mesa6"), 0, 1, 'C');
    //$pdf->Ln(4); Saltar una línea después de la información de la Mesa 6 (DESCOMENTAR PARA UTILIZAR)
} else {
    // Mensaje de error si no se encontró la Mesa 6
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Error: Mesa 6 no encontrada"), 0, 1, 'C');
    $pdf->Ln(4); // Saltar una línea después del mensaje de error
}
...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('TPV: Terraza'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('En Terraza'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
...
$sql = "SELECT nombre, cantidad, preciou FROM ordenmesa6";
...
$sql_total = "SELECT SUM(preciou) AS total FROM ordenmesa6";
...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Cel/Whats: 2441440564'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
