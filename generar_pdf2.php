...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Pedido: MESA 2'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
...
// Obtener información de la "Mesa 2" de la tabla "mesas"
$sql_mesa2 = "SELECT tipo, habitacion, comensal FROM mesas WHERE id = 2";
$result_mesa2 = $conn->query($sql_mesa2);

// Verificar si se obtuvieron resultados
if ($result_mesa2->num_rows > 0) {
    $row_mesa2 = $result_mesa2->fetch_assoc();
    $tipo_mesa2 = $row_mesa2['tipo'];
    $habitacion_mesa2 = $row_mesa2['habitacion'];
    $comensal_mesa2 = $row_mesa2['comensal'];

    // Mostrar la información en el ticket
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("$tipo_mesa2: $habitacion_mesa2"), 0, 1, 'C');
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Cliente: $comensal_mesa2"), 0, 1, 'C');
    //$pdf->Ln(4); Saltar una línea después de la información de la Mesa 2 (DESCOMENTAR PARA UTILIZAR)
} else {
    // Mensaje de error si no se encontró la Mesa 2
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Error: Mesa 2 no encontrada"), 0, 1, 'C');
...
$sql = "SELECT nombre, cantidad, preciou FROM ordenmesa2";
...
$sql_total = "SELECT SUM(preciou) AS total FROM ordenmesa2";
...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Cel/Whats: 2441440524'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
