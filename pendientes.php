...
if(!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true){
    ...
    $sql="SELECT * FROM ordenes_listas";
...
<html lang="en">
    <head>
        <title>Órdenes Listas - Ocaso Terraza</title>
        ...
        <style>
            ...
            .card {
        border: 2px solid #000; /* Cambiar el color y el grosor del borde según tus preferencias */
        border-radius: 10px; /* Agregar bordes redondeados a la tarjeta */
        margin-bottom: 20px; /* Agregar espacio entre las tarjetas */
        }

        .card-title {
            font-size: 24px; /* Cambiar el tamaño del título según tus preferencias */
    ...
    <div class="card-body text-center">
        <h3 class="card-title">Orden no.<?php echo $row['id'] ?> </h3>
        <p class="card-text">Platillo: <?php echo $row['nombre'] ?></p>
        <p class="card-text">Mesa: <?php echo $row['mesa'] ?></p>
        
        <?php
        // Formatear la fecha y hora
        $datetime = new DateTime($row['fyh']);
        $formattedDateTime = $datetime->format('h:i A d/m/Y');
        ?>

        <p class="card-text">Fecha y Hora: <?php echo $formattedDateTime ?></p>
        
        <a href="<?php echo $enlace ?>" class="btn btn-primary"><i class="fa1 fa fa-plus"></i></a>
        <br><br>
        <a href="atender_orden.php?id=<?php echo $row['id'] ?>" class="btn btn-danger"><b>Finish</b></a>
    </div>
</div>
