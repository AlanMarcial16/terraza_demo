...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Pedido: MESA 5'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
...
// Obtener información de la "Mesa 5" de la tabla "mesas"
$sql_mesa5 = "SELECT tipo, habitacion, comensal FROM mesas WHERE id = 5";
$result_mesa5 = $conn->query($sql_mesa5);

// Verificar si se obtuvieron resultados
if ($result_mesa5->num_rows > 0) {
    $row_mesa5 = $result_mesa5->fetch_assoc();
    $tipo_mesa5 = $row_mesa5['tipo'];
    $habitacion_mesa5 = $row_mesa5['habitacion'];
    $comensal_mesa5 = $row_mesa5['comensal'];

    // Mostrar la información en el ticket
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("$tipo_mesa5: $habitacion_mesa5"), 0, 1, 'C');
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Cliente: $comensal_mesa5"), 0, 1, 'C');
    //$pdf->Ln(4); Saltar una línea después de la información de la Mesa 5 (DESCOMENTAR PARA UTILIZAR)
} else {
    // Mensaje de error si no se encontró la Mesa 5
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Error: Mesa 5 no encontrada"), 0, 1, 'C');
...
$sql = "SELECT nombre, cantidad, preciou FROM ordenmesa5";
...
$sql_total = "SELECT SUM(preciou) AS total FROM ordenmesa5";
...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Cel/Whats: 2441440554'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
