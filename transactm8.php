...
$conn8 = mysqli_connect($servername, $username, $password, "pruebar_demo");
...
// Obtener la habitación de Mesa 8 de la base de datos "pruebar_demo"
$sql = "SELECT habitacion FROM mesas WHERE nombre = 'Mesa 8'";
$result = mysqli_query($conn8, $sql);

if (mysqli_num_rows($result) > 0) {
    // Obtener el valor de "habitación" de Mesa 8
    ...
    if (mysqli_num_rows($resultReservaciones) > 0) {
        ...
        // Consulta SQL para obtener la suma de 'preciou' de la tabla 'ordenmesa8' en la base de datos "pruebar_demo"
        $sqlTotal = "SELECT SUM(preciou) AS total FROM ordenmesa8";
        ...
        if (mysqli_num_rows($resultTotal) > 0) {
            ...
            if (mysqli_query($conn2, $updateSql)) {
                ...
                header("Location: guardaticket8.php");
                exit; // Asegurarse de que el script se detenga aquí
            } else {
                echo "Error al actualizar 'cextras': " . mysqli_error($conn2);
            }
        } else {
            echo "No se pudo obtener el valor de 'total' desde la tabla 'ordenmesa8' en la base de datos 'pruebar_demo'.";
        }
    ...
    echo "No se encontró la habitación 'Mesa 8' en la tabla 'mesas' de la base de datos 'pruebar_demo'.";
}
?>
