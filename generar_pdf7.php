...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Pedido: MESA 7'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del 
...
// Obtener información de la "Mesa 7" de la tabla "mesas"
$sql_mesa7 = "SELECT tipo, habitacion, comensal FROM mesas WHERE id = 7";
$result_mesa7 = $conn->query($sql_mesa7);

// Verificar si se obtuvieron resultados
if ($result_mesa7->num_rows > 0) {
    $row_mesa7 = $result_mesa7->fetch_assoc();
    $tipo_mesa7 = $row_mesa7['tipo'];
    $habitacion_mesa7 = $row_mesa7['habitacion'];
    $comensal_mesa7 = $row_mesa7['comensal'];

    // Mostrar la información en el ticket
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("$tipo_mesa7: $habitacion_mesa7"), 0, 1, 'C');
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Cliente: $comensal_mesa7"), 0, 1, 'C');
    //$pdf->Ln(4); Saltar una línea después de la información de la Mesa 7 (DESCOMENTAR PARA UTILIZAR)
} else {
    // Mensaje de error si no se encontró la Mesa 7
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Error: Mesa 7 no encontrada"), 0, 1, 'C');
...
$sql = "SELECT nombre, cantidad, preciou FROM ordenmesa7";
...
$sql_total = "SELECT SUM(preciou) AS total FROM ordenmesa7";
...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Cel/Whats: 2441440574'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
