...
if (isset($_POST['id'])) {
    ...
    if (is_numeric($id)) {
        ...
        $eliminar_info_query = mysqli_query($conn, "SELECT nombre FROM ordenMesa8 WHERE id = $id");
        $eliminar_info_row = mysqli_fetch_assoc($eliminar_info_query);
        $nombre_eliminar = $eliminar_info_row['nombre'];

        $sql_eliminar_ordenMesa8 = "DELETE FROM ordenMesa8 WHERE id = $id";
        if (mysqli_query($conn, $sql_eliminar_ordenMesa8)) {
            ...
            $total_query = mysqli_query($conn, "SELECT SUM(preciou) AS total FROM ordenMesa8");
