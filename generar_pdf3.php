...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Pedido: MESA 3'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
...
// Obtener información de la "Mesa 3" de la tabla "mesas"
$sql_mesa3 = "SELECT tipo, habitacion, comensal FROM mesas WHERE id = 3";
$result_mesa3 = $conn->query($sql_mesa3);

// Verificar si se obtuvieron resultados
if ($result_mesa3->num_rows > 0) {
    $row_mesa3 = $result_mesa3->fetch_assoc();
    $tipo_mesa3 = $row_mesa3['tipo'];
    $habitacion_mesa3 = $row_mesa3['habitacion'];
    $comensal_mesa3 = $row_mesa3['comensal'];

    // Mostrar la información en el ticket
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("$tipo_mesa3: $habitacion_mesa3"), 0, 1, 'C');
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Cliente: $comensal_mesa3"), 0, 1, 'C');
    //$pdf->Ln(4); Saltar una línea después de la información de la Mesa 3 (DESCOMENTAR PARA UTILIZAR)
} else {
    // Mensaje de error si no se encontró la Mesa 3
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Error: Mesa 3 no encontrada"), 0, 1, 'C');
...
$sql = "SELECT nombre, cantidad, preciou FROM ordenmesa3";
...
$sql_total = "SELECT SUM(preciou) AS total FROM ordenmesa3";
...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Cel/Whats: 2441440534'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
