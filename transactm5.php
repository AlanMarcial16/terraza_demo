...
$conn5 = mysqli_connect($servername, $username, $password, "pruebar_demo");
...
// Obtener la habitación de Mesa 5 de la base de datos "pruebar_demo"
$sql = "SELECT habitacion FROM mesas WHERE nombre = 'Mesa 5'";
$result = mysqli_query($conn5, $sql);

if (mysqli_num_rows($result) > 0) {
    // Obtener el valor de "habitación" de Mesa 5
    ...
    if (mysqli_num_rows($resultReservaciones) > 0) {
        ...
        // Consulta SQL para obtener la suma de 'preciou' de la tabla 'ordenmesa5' en la base de datos "pruebar_demo"
        $sqlTotal = "SELECT SUM(preciou) AS total FROM ordenmesa5";
        ...
        if (mysqli_num_rows($resultTotal) > 0) {
            ...
            if (mysqli_query($conn2, $updateSql)) {
                ...
                header("Location: guardaticket5.php");
                exit; // Asegurarse de que el script se detenga aquí
            } else {
                echo "Error al actualizar 'cextras': " . mysqli_error($conn2);
            }
        } else {
            echo "No se pudo obtener el valor de 'total' desde la tabla 'ordenmesa5' en la base de datos 'pruebar_demo'.";
        }
    ...
    echo "No se encontró la habitación 'Mesa 5' en la tabla 'mesas' de la base de datos 'pruebar_demo'.";
}
?>
