...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Pedido: MESA 8'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
...
// Obtener información de la "Mesa 8" de la tabla "mesas"
$sql_mesa8 = "SELECT tipo, habitacion, comensal FROM mesas WHERE id = 8";
$result_mesa8 = $conn->query($sql_mesa8);

// Verificar si se obtuvieron resultados
if ($result_mesa8->num_rows > 0) {
    $row_mesa8 = $result_mesa8->fetch_assoc();
    $tipo_mesa8 = $row_mesa8['tipo'];
    $habitacion_mesa8 = $row_mesa8['habitacion'];
    $comensal_mesa8 = $row_mesa8['comensal'];

    // Mostrar la información en el ticket
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("$tipo_mesa8: $habitacion_mesa8"), 0, 1, 'C');
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Cliente: $comensal_mesa8"), 0, 1, 'C');
    //$pdf->Ln(4); Saltar una línea después de la información de la Mesa 8 (DESCOMENTAR PARA UTILIZAR)
} else {
    // Mensaje de error si no se encontró la Mesa 8
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Error: Mesa 8 no encontrada"), 0, 1, 'C');
...
$sql = "SELECT nombre, cantidad, preciou FROM ordenmesa8";
...
$sql_total = "SELECT SUM(preciou) AS total FROM ordenmesa8";
...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Cel/Whats: 2441440584'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
