...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Pedido: MESA 4'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
...
// Obtener información de la "Mesa 4" de la tabla "mesas"
$sql_mesa4 = "SELECT tipo, habitacion, comensal FROM mesas WHERE id = 4";
$result_mesa4 = $conn->query($sql_mesa4);

// Verificar si se obtuvieron resultados
if ($result_mesa4->num_rows > 0) {
    $row_mesa4 = $result_mesa4->fetch_assoc();
    $tipo_mesa4 = $row_mesa4['tipo'];
    $habitacion_mesa4 = $row_mesa4['habitacion'];
    $comensal_mesa4 = $row_mesa4['comensal'];

    // Mostrar la información en el ticket
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("$tipo_mesa4: $habitacion_mesa4"), 0, 1, 'C');
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Cliente: $comensal_mesa4"), 0, 1, 'C');
    //$pdf->Ln(4); Saltar una línea después de la información de la Mesa 4 (DESCOMENTAR PARA UTILIZAR)
} else {
    // Mensaje de error si no se encontró la Mesa 4
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Error: Mesa 4 no encontrada"), 0, 1, 'C');
...
$sql = "SELECT nombre, cantidad, preciou FROM ordenmesa4";
...
$sql_total = "SELECT SUM(preciou) AS total FROM ordenmesa4";
...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Cel/Whats: 2441440544'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
