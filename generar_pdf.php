...
$pdf->SetX(($pdf->GetPageWidth() - 58) / 2); // Centrar horizontalmente los datos del hotel
$pdf->Cell(58, 5, utf8_decode('Pedido: MESA 1'), 0, 1, 'C'); // Centrar el texto y saltar una línea después del texto
...
// Obtener información de la "Mesa 1" de la tabla "mesas"
$sql_mesa1 = "SELECT tipo, habitacion, comensal FROM mesas WHERE id = 1";
$result_mesa1 = $conn->query($sql_mesa1);

// Verificar si se obtuvieron resultados
if ($result_mesa1->num_rows > 0) {
    $row_mesa1 = $result_mesa1->fetch_assoc();
    $tipo_mesa1 = $row_mesa1['tipo'];
    $habitacion_mesa1 = $row_mesa1['habitacion'];
    $comensal_mesa1 = $row_mesa1['comensal'];

    // Mostrar la información en el ticket
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("$tipo_mesa1: $habitacion_mesa1"), 0, 1, 'C');
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Cliente: $comensal_mesa1"), 0, 1, 'C');
    //$pdf->Ln(4); Saltar una línea después de la información de la Mesa 1 (DESCOMENTAR PARA UTILIZAR)
} else {
    // Mensaje de error si no se encontró la Mesa 1
    $pdf->SetX(($pdf->GetPageWidth() - 58) / 2);
    $pdf->Cell(58, 5, utf8_decode("Error: Mesa 1 no encontrada"), 0, 1, 'C');
...
$sql = "SELECT nombre, cantidad, preciou FROM ordenMesa1";
...
$sql_total = "SELECT SUM(preciou) AS total FROM ordenMesa1";
